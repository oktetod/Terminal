...
Fixed: Load LoRA from JSON files, consistent API key handling, GPU L4
"""
...
def load_lora_models_from_json(json_dir="/json_data"):
    """
    Load all LoRA models from JSON files (01.json to 07.json)
    Returns a dictionary with all LoRA configurations
    """
    lora_models = {}
    json_files = ["01.json", "02.json", "03.json", "04.json", "05.json", "06.json", "07.json"]
...
# Mount JSON files dari direktori lokal
json_mount = modal.Mount.from_local_dir(
    ".",  # Current directory
    remote_path="/json_data",
    condition=lambda path: path.endswith(".json")
)
...
def download_loras():
    ...
    lora_models = load_lora_models_from_json("/json_data")
...
class ModelInference:
    ...
    def load_model(self):
        ...
        self.lora_models = load_lora_models_from_json("/json_data")
...
def fastapi_app():
    ...
    async def root():
        return {
            ...
            "total_loras": len(lora_models),
            "available_loras": list(lora_models.keys())[:20],  # Show first 20 for brevity
