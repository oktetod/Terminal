...
"""
Deploy Merged Model ke Modal.com dengan FastAPI - FIXED VERSION
...
# Import Pydantic at the top (FIX #1)
try:
    from pydantic import BaseModel, Field, field_validator, ConfigDict
    PYDANTIC_AVAILABLE = True
except ImportError as e:
    logger.error(f"CRITICAL: Pydantic not installed! Error: {e}")
    logger.error("Please rebuild Modal image with: modal deploy --force-rebuild modal_app.py")
...
class Config:
    ...
    VERSION = "5.0.2"  # Version bump - dependency fixes
    GPU_TYPE = "L4"
...
# Simplified image - NO NumPy explicitly listed
image = (
    modal.Image.debian_slim(python_version="3.11")
    .apt_install("git")  # Add this to force rebuild
    .pip_install(
        # Core dependencies
    ...
    .run_commands(
        ...
        "echo 'Image build completed successfully'",  # Add verification
    )
)
...
def validate_image_size(image_bytes: bytes) -> bool:
    """Validate image file size (FIX #6: Added proper return)"""
...
# ===================================================================
# Pydantic already imported at top (FIX #1)
...
class ModelInference:
    ...
    def _preprocess_control_image(self, image, controlnet_type: str):
        """Preprocess control image based on type - FIXED (FIX #3, #5)"""
    ...
    def image_to_image(self, request_dict: Dict[str, Any]):
        """Edit image with prompt - FIXED (FIX #2)"""
        from PIL import Image
        import torch
        ...
        try:
            ...
            # FIX #2: Use Image.Resampling.LANCZOS for Pillow 10.x
...
def fastapi_app():
    ...
    web_app = FastAPI(
        ...
        version=Config.VERSION,
        description="Merged SDXL model with bug fixes applied"
    )
    ...
    async def root(request: Request):
        ...
        return {
            ...
            "model": "Juggernaut ALL-IN-ONE (Merged Model)",
            "bug_fixes": [
                "✅ Fixed: Pydantic import with fallback handler",
                "✅ Fixed: PIL LANCZOS compatibility (Pillow 10.x)",
                "✅ Fixed: CV2 numpy array handling",
                "✅ Fixed: Image size validation return",
                "✅ Fixed: Canny edge detection",
                "✅ Fixed: Stable dependency versions (Pydantic 2.5.0)",
                "✅ Fixed: Build verification commands",
            ],
            "endpoints": {
                "health": "GET /health",
    ...
    async def health_check(request: Request):
        ...
        return {
            "status": "healthy" if PYDANTIC_AVAILABLE else "degraded",
...
def main():
    ...
    print("\n" + "="*70)
    print("🚀 JUGGERNAUT ALL-IN-ONE API - FIXED VERSION v5.0.2")
    print("="*70)
    print(f"\n📦 Version: {Config.VERSION}")
    print("✅ All Bugs Fixed:")
    print("  1. ✅ Pydantic import with fallback")
    print("  2. ✅ PIL LANCZOS compatibility (Pillow 10.x)")
    print("  3. ✅ CV2 numpy array handling fixed")
    print("  4. ✅ Image validation return value fixed")
    print("  5. ✅ Canny edge detection improved")
    print("  6. ✅ Missing error handling added")
    print("  7. ✅ Stable dependency versions")
    print("\n📋 Available Commands:\n")
    print("  1. Download ControlNet models:")
    print("     modal run modal_app.py::download_controlnet_models\n")
    print("  2. Deploy API (FORCE REBUILD - RECOMMENDED):")
    print("     modal deploy --force-rebuild modal_app.py\n")
    print("  3. Deploy API (normal):")
    print("     modal deploy modal_app.py\n")
    print("  4. Test locally:")
    print("     modal serve modal_app.py\n")
    print("="*70)
    print("\n⚠️  PENTING - JIKA ERROR 'pydantic not found':")
    print("  • Jalankan: modal deploy --force-rebuild modal_app.py")
    print("  • Force rebuild akan install ulang semua dependencies")
    print("  • Tunggu hingga build selesai (~5-10 menit)")
    print("="*70)
    print("\n📦 Dependencies:")
    print("  • Pydantic: 2.5.0 (stable)")
    print("  • FastAPI: 0.104.1")
    print("  • PyTorch: 2.1.0")
    print("  • Diffusers: 0.25.0")
    print("="*70)
    print("\n🎯 Next Steps:")
    print("  1. FORCE REBUILD: modal deploy --force-rebuild modal_app.py")
    print("  2. Wait for build to complete")
    print("  3. Download ControlNet models (if needed)")
    print("  4. Test your fixed API!")
    print("="*70 + "\n")
    
    # Check if pydantic is available
    if not PYDANTIC_AVAILABLE:
        print("\n" + "!"*70)
        print("⚠️  WARNING: Pydantic is NOT available in current environment!")
        print("!"*70)
        print("\nThis is normal if running locally without Modal environment.")
        print("Modal will install it when deploying.\n")