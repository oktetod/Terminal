...
# ===================================================================
@app.function(
    image=image.copy_local_dir(".", remote_path="/json_files", condition=lambda p: p.endswith(".json")),
    volumes={JSON_DATA_DIR: json_volume}, 
    timeout=600
)
def upload_json_files():
    """Upload JSON configuration files to volume"""
    from pathlib import Path
    import shutil
    
    local_files = ["01.json", "03.json", "04.json", "05.json", "07.json"]
    
    for json_file in local_files:
        source = Path("/json_files") / json_file
        if source.exists():
            dest = Path(JSON_DATA_DIR) / json_file
            dest.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy(source, dest)
            print(f"✓ Uploaded {json_file}")
        else:
            print(f"⚠ {json_file} not found at {source}")
    
    json_volume.commit()
    print("✓ All JSON files uploaded to volume")
...
def main():
    ...
    print("  modal run modal_app.py::upload_json_files")
